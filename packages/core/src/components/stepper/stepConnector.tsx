...
import { Classes, Intent, IProps } from "../../common";

export interface IStepConnectorProps extends IProps {
    /**
     * Gives the connector an active appearance.  If using `Stepper`, this is defaulted to true if `activeStep` prop matches following step's index.
     */
    active?: boolean;
...
@polyfill
export class StepConnector extends React.PureComponent<IStepConnectorProps & React.HTMLAttributes<HTMLDivElement>> {
    public static readonly displayName = "StepConnector";

    public render() {
        return (
            <div className={Classes.STEP_CONNECTOR}>
                <span className={Classes.STEP_CONNECTOR_LINE} />
            </div>
        );
    }
}
