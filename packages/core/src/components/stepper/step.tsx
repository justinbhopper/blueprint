...
import { Classes, Intent, IProps, MaybeElement } from "../../common";
...
export interface IAttachedStep {
...
@polyfill
export class Step extends React.PureComponent<IStepProps & IAttachedStep & React.HTMLAttributes<HTMLDivElement>> {
    public render() {
        const {
            children,
            className,
        ...
        return (
            <div className={classNames(className, Classes.STEP, stateClasses)}>
                ...
                <span className={Classes.STEP_LABEL_ROOT} {...divProps}>
                    {icon ? (
                        ...
                        <span style={iconRootStyles} className={classNames(Classes.STEP_ICON_DOT_ROOT, stateClasses)}>
                            <div className={classNames(Classes.STEP_ICON_DOT, stateClasses, intentClass(intent))} />
                        </span>
                    )}
                ...
                {children && (
                    <Collapse
                        isOpen={active}
                        className={classNames(Classes.STEP_CONTENT, contentClassName)}
                        {...restContentProps}
                    >
                        {children}
                    </Collapse>
